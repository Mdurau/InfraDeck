...
// ==============================================================================
// 1. FIXED: Component extracted OUTSIDE of the parent render to prevent resetting DOM state
// ==============================================================================
...
const ContainerTable = ({ list, isRunningGroup, onAction }: TableProps) => (
          ...
          list.map((container) => (
            <tr key={container.id} className="hover:bg-slate-850/40">
              ...
              <td className="py-3 text-right space-x-2">
                {isRunningGroup ? (
                  <>
                    <button onClick={() => onAction(container.id, 'stop')} className="px-3 py-1 bg-rose-600/90 hover:bg-rose-500 rounded text-xs font-medium text-white transition">
                      Stop
                    </button>
                    <button onClick={() => onAction(container.id, 'restart')} className="px-3 py-1 bg-slate-800 hover:bg-slate-700 rounded text-xs font-medium text-slate-300 transition">
                      Restart
                    </button>
                  </>
                ) : (
                  <button onClick={() => onAction(container.id, 'start')} className="px-3 py-1 bg-emerald-600/90 hover:bg-emerald-500 rounded text-xs font-medium text-white transition">
                    Start
                  </button>
                )}
              </td>
    ...
    <main className="min-h-screen bg-slate-950 text-slate-50 p-8">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
          ...
          <p className="text-xs text-slate-400 mt-1 font-mono">{metrics?.cpu_cores ?? 0} <span className="text-slate-500">Available Cores</span></p>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
          ...
          <p className="text-xs text-slate-400 mt-1 font-mono">{metrics?.memory.used_gb ?? 0} GB <span className="text-slate-600">/</span> {metrics?.memory.total_gb ?? 0} GB</p>
          <div className="w-full bg-slate-800 h-2 rounded-full mt-3 overflow-hidden">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
          ...
          <p className="text-xs text-slate-400 mt-1 font-mono">{metrics?.disk.used_gb ?? 0} GB <span className="text-slate-600">/</span> {metrics?.disk.total_gb ?? 0} GB</p>
          <div className="w-full bg-slate-800 h-2 rounded-full mt-3 overflow-hidden">
      ...
      <section>
        <ContainerTable list={runningContainers} isRunningGroup={true} onAction={handleAction} />
        <ContainerTable list={exitedContainers} isRunningGroup={false} onAction={handleAction} />
      </section>
    </main>
  );
}